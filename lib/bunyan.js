...
RotatingFileStream = function RotatingFileStream(options) {
    ...
    // force to daily rotate, and won't care about count
    this.dateExt = options.dateExt;
...
RotatingFileStream.prototype.rotate = function rotate() {
    ...
    function rotateWithDateExt() {
        var date = new Date();
        date = new Date(date.getTime() - 86400 * 1e3);
        var year = date.getFullYear(),
            month = (date.getMonth() + 1),
            day = date.getDate();
        month = month < 10 ? '0' + month : month;
        day = day < 10 ? '0' + day : day;
        var toPath = self.path + '-' + year + '-' + month + '-' + day;

        fs.exists(toPath, function (exists) {
            if (exists) {
                if (_DEBUG) {
                    console.log('[pid %s] %s already exists, will not rotate', process.pid, toPath);
                }
                finish();
            } else {
                if (_DEBUG) {
                    console.log('[pid %s] mv %s %s', process.pid, self.path, toPath);
                }
                mv(self.path, toPath, function (mvErr) {
                    if (mvErr) {
    ...
    // remove the last file, and move previous one to last
    function del() {
